...
"""
    ...
    def get_features_from_registry(self, project_name: str) -> Tuple[List[FeatureAnchor], List[DerivedFeature]]:
        ...
        entities_under_project = self._list_registered_entities_with_details(project_name=project_name,entity_type=[TYPEDEF_DERIVED_FEATURE, TYPEDEF_ANCHOR_FEATURE, TYPEDEF_FEATHR_PROJECT, TYPEDEF_ANCHOR, TYPEDEF_SOURCE])
        ...
        for anchor_entity in [x for x in entities_under_project if x['typeName']==TYPEDEF_ANCHOR]:
            items_under_anchor = [entities_dict[x[1]] for x in consume_relations if x[0]==anchor_entity['guid']]
            anchor_features_guid = [x['guid'] for x in items_under_anchor if x['typeName']==TYPEDEF_ANCHOR_FEATURE]
            source_entity = [x for x in items_under_anchor if x['typeName']==TYPEDEF_SOURCE][0]
